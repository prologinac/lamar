...
if (userMessage.startsWith('.') && message.key.fromMe) {
    if (!userMessage.startsWith('.type')) { // Don't break the typewriter
        try {
            await sock.sendMessage(chatId, { 
                text: "Ѷ", 
                edit: message.key 
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.kick'):
                ...
                break;
            case userMessage.startsWith('.pp') || userMessage === '.profile':
    {
        // Extract arguments (like the phone number)
        const args = userMessage.split(' ').slice(1);
        await profileCommand(sock, chatId, message, args);
        commandExecuted = true;
    }
    break;
    ...
    break;
            ...
            case userMessage.startsWith('.antibadword'):
                ...
                if (text.startsWith('.savestatus')) {
    // Make sure the user is quoting a status
    if (!message.message.extendedTextMessage?.contextInfo?.quotedMessage) {
        await sock.sendMessage(chatId, { text: "❌ Please quote a status to save." });
    } else {
        await saveStatus(sock, chatId, message);
    }
}
                if (!isBotAdmin) {
